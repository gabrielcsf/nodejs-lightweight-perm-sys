...
Module.prototype.require = function(path) {
  ...
  // sandbox module export functions
  var invertedModule = invert(module);
  ...
  var keys = Object.keys(module);
    for (var i = 0, length = keys.length; i < length; i++) {
      ...
      if (typeof module[keys[i]] === 'function') {
        var p = new Proxy(module[keys[i]], { 
          apply : function(target, thisArg, argumentsList) {
          console.log("Calling function: " + invertedModule[target]);
          }
        });
        sandboxedModule[keys[i]] = p;
      } else {
        sandboxedModule[keys[i]] = module[keys[i]];
      }
