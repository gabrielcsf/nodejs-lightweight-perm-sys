...
const moduleDependenciesPermissions = {};
Module._modulePermissions = function() {
  return moduleDependenciesPermissions;
}
...
Module.prototype.require = function(path) {
  ...
  // var whiteList = [ 'os', 'crypto', 'stream', 'path', 'timers', 'constants', 'assert', 'lodash', 'safe-buffer', 'buffer-dataview' ];
...
Module.prototype._compile = function(content, filename) {
  ...
  require = Module.wrapRequire(require, dirname, filename);
  // --------------------------------------------------------------------------
...
Module.wrapRequire = function(require, dirname, filename) {
  ...
  return function(moduleName) {
    ...
    console.log(sourceModule + " LOAD-MODULE " + targetModule);
    if (sourceModule !== "(node-application)" && sourceModule !== targetModule && Module.isCoreLib(targetModule)) {

      var dirnameArray = dirname.split("/");
      var sourceModulePermRootDir = dirnameArray.slice(0, dirnameArray.indexOf(sourceModule)+1).join("/");

        //console.log("Loading permissions for module: " + sourceModule + " (dir: " + modulePermRootDir + ")");

        Module.checkPermission(sourceModule, targetModule, sourceModulePermRootDir);
      }
...
Module.checkPermission = function(sourceModule, targetModule, sourceModuleRootDir) {
  ...
  try {
    ...
    console.log("No permission file: " + sourceModulePermFile);
  }
  var noPermissionMessage = "Module " + sourceModule + " does not have permission to require module " + targetModule;

  if (targetModule && Array.isArray(sourceModulePermissions)) {
    var hasPermissions = sourceModulePermissions.includes(targetModule);
    if (!hasPermissions) {
      console.log(noPermissionMessage);
    }
  }
};
