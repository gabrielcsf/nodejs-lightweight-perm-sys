...
membraneFactory.create = function(initTarget) {
  var revoked = false;
  ...
  var wrap = function(target) {
    ...
    var metaHandler = {
      get : function (dummyTarget, propertyName) {
       ...
       apply : function(dummyTarget, thisArg, argumentsList) {
        if (revoked) { throw new Error("membrane revoked"); }
    ...
    var wrapper = new Proxy(target, new Proxy(dummyTarget, metaHandler));
    return wrapper;
  }
...
Module.prototype.require = function(path) {
  ...
  //console.log("Wrapping module Name:  " + path);
  return membraneFactory.create(module).ref;
  //return module;
};
