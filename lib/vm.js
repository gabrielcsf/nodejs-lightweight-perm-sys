...
const realRunInProxiedContext = Script.prototype.runInProxiedContext;
...
Script.prototype.runInProxiedContext = function(globalObject, options) {
  if (options && options.breakOnSigint) {
    return sigintHandlersWrap(() => {
      return realRunInProxiedContext.call(this, globalObject, options);
    });
  } else {
    return realRunInProxiedContext.call(this, globalObject, options);
  }
...
Script.prototype.runInSameContext = function(globalObject, options) {
  var context = exports.reuseContext(globalObject);
  return this.runInContext(context, options);
};
...
exports.reuseContext = function(sandbox) {
  if (sandbox === undefined) {
    sandbox = {};
  } else if (binding.isContext(sandbox)) {
    return sandbox;
  }
  binding.reuseContext(sandbox);
  return sandbox;
};
...
exports.runInProxiedContext = function(code, sandbox, options) {
  var script = new Script(code, options);
  return script.runInProxiedContext(sandbox, options);
};

exports.runInSameContext = function(code, sandbox, options) {
  var script = new Script(code, options);
  return script.runInSameContext(sandbox, options);
};
